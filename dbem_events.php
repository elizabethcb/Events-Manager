...
/* Marcus End Edit */
	...
	if (preg_match ( "/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/", $scope )) {
		$conditions [] = " start_date like '$scope'";
	} else {
		...
		/* Marcus Begin Edit */
			...
			$conditions [] = "(start_date >= '$today' )"; //OR (event_end_date >= '$today' AND event_end_date != '0000-00-00' AND event_end_date IS NOT NULL))";
		/* Marcus End Edit */
		if ($scope == "past")
	...
	$sql = "SELECT event_id, 
			    name, 
			  	DATE_FORMAT(start_time, '%e') AS 'event_day',
			  	DATE_FORMAT(start_time, '%Y') AS 'event_year',
			  	DATE_FORMAT(start_time, '%k') AS 'event_hh',
			  	DATE_FORMAT(start_time, '%i') AS 'event_mm',
				DATE_FORMAT(end_time, '%e') AS 'event_end_day',
			  	DATE_FORMAT(end_time, '%Y') AS 'event_end_year',
			  	DATE_FORMAT(end_time, '%k') AS 'event_end_hh',
			  	DATE_FORMAT(end_time, '%i') AS 'event_end_mm',
			  	start_date,
					...
					FROM $events_table   
					$where
					ORDER BY start_date $order, start_time $order
					$limit 
					$offset";
...
<h2><?php
	...
	$link = array ();
	$link ['past'] = "<a href='" . get_bloginfo ( 'url' ) . "/wp-admin/admin.php?page=events-manager2/events-manager2.php&amp;scope=past&amp;order=desc'>" . __ ( 'Past events', 'dbem' ) . "</a>";
	$link ['all'] = " <a href='" . get_bloginfo ( 'url' ) . "/wp-admin/admin.php?page=events-manager2/events-manager2.php&amp;scope=all&amp;order=desc'>" . __ ( 'All events', 'dbem' ) . "</a>";
	$link ['future'] = "  <a href='" . get_bloginfo ( 'url' ) . "/wp-admin/admin.php?page=events-manager2/events-manager2.php&amp;scope=future'>" . __ ( 'Future events', 'dbem' ) . "</a>";
	...
	name='page' value='events-manager2/events-manager2.php' />
<ul class="subsubsub">
	...
	<tbody>
	  ...
	  <tr <?php
			...
			<td><strong><a class="row-title"
				href="<?php
			bloginfo ( 'url' )?>/wp-admin/admin.php?page=events-manager2/events-manager2.php&amp;action=edit_event&amp;event_id=<?php
			echo $event ['event_id'];
			?>"><?php
			...
			<td>
 	    	...
 	    	<a
				href="<?php
			bloginfo ( 'url' )?>/wp-admin/admin.php?page=events-manager2/events-manager2.php&amp;action=duplicate_event&amp;event_id=<?php
			echo $event ['event_id'];
			?>"
				title="<?php
  	     ...
  	     <?php
			...
			<a
				href="<?php
				bloginfo ( 'url' )?>/wp-admin/admin.php?page=events-manager2/events-manager2.php&amp;action=edit_recurrence&amp;recurrence_id=<?php
...
<?php
	if ($events_count > $limit) {
		...
		if ($backward < $events_count)
			echo "<a style='float: left' href='" . get_bloginfo ( 'url' ) . "/wp-admin/admin.php?page=events-manager2/events-manager2.php&amp;scope=$scope&offset=$backward'>&lt;&lt;</a>";
		if ($forward >= 0)
			echo "<a style='float: right' href='" . get_bloginfo ( 'url' ) . "/wp-admin/admin.php?page=events-manager2/events-manager2.php&amp;scope=$scope&offset=$forward'>&gt;&gt;</a>";
		echo "</div>";
	}
	?>
...
function dbem_event_form($event, $title, $element) {
	...
	$_GET ['action'] == "edit_recurrence" ? $form_destination = get_bloginfo('url') . "wp-admin/admin.php?page=events-manager2/events-manager2.php&amp;action=update_recurrence&amp;recurrence_id=" . $element : $form_destination = "admin.php?page=events-manager2/events-manager2.php&amp;action=update_event&amp;event_id=" . $element;
...
function dbem_enque_scripts() {
	...
	// wp_enqueue_script('datepicker','/wp-content/plugins/events-manager2/jquery-ui-datepicker/jquery-ui-personalized-1.6b.js', array('jquery') );
}
...
function dbem_admin_options_script() {
	if (isset ( $_REQUEST ['page'] ) && $_REQUEST ['page'] == 'admin/events-admin.php') {
		?>
<script type="text/javascript">
	//<![CDATA[
		$j=jQuery.noConflict();
...
function dbem_general_css() {
	...
	if (is_page("Events")) {
		echo '<link rel="stylesheet" href="' . DBEM_URI . 'css/events_manager.css" type="text/css" />';
	}
}
...
// Moved to events admin action_delete_events
